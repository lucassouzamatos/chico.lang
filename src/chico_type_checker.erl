...
infer(?is_integer, Env) -> {anotate(number), Env};
infer(?is_float, Env) -> {anotate(number), Env};
infer(?is_string, Env) -> {anotate(string), Env};

infer({declaration, _, Name}, Env) ->
  case lookup_var(Name, Env) of
    {value, {_, Type}} -> {Type, Env};
    _ -> warning_message("Let " ++ atom_to_list(Name) ++ " was not found.")
  end;
